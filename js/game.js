...
function Game (savedGame) {
  ...
  var gameTime = 0; var moves = []; var gameState; var whitesTurn = 1; var castlingAllowed = { white: { queenSide: true, kingSide: true }, black: { queenSide: true, kingSide: true } }

  // Environment variables that can be recalculated in runtime
  var field = {}; var timerInterval; var showCaptionInterval; var gameOver; var clickedPieceIsMoving
  ...
  function updateInfoDrawer () {
    ...
    $('#currentTurn > span:first-child').html(currentTurnString)
  ...
  function pieceObject (element, object) {
    if (!object) return $(element).data('pieceObject')
    else return $(element).data('pieceObject', object)
  }

  function fieldObject (element, object) {
    if (!object) return $(element).data('fieldObject')
    else return $(element).data('fieldObject', object)
  }
  ...
  function finishMove () {
    ...
    if (!possibleTurn) {
      if (moves[moves.length - 1].check) {
        ...
        // Update end menu
        $('#end > .tone100 > h2 > span:first-child').css('background-position', (whitesTurn) ? '-180px 100%' : '0 100%')
        ...
        // Update end menu
        $('#end > .tone100 > h2 > span:first-child').css('background-position', '-360px 100%')
  ...
  function Piece (pieceElement) {
    ...
    this.getReachableFields = function () {
      ...
      var fields = new Fields(this.field.column, this.field.row); var forward = (that.isWhite) ? +1 : -1; var i
    ...
    this.captureableFields = function () {
      var otherFields = this.reachableFields; var fields = []
    ...
    this.updateReachables = function () {
      // Set field classes used for awareness mode
      function updatePropertyClass (fld) {
        if (fld.reachableBy.white.length !== fld.reachableBy.black.length) fld.element.className = 'property' + (fld.reachableBy.white.length - fld.reachableBy.black.length)
        else fld.element.className = 'neutral' + fld.reachableBy.white.length
      }
    ...
    this.startDragging = function () {
      ...
      $('#board td').removeClass('acceptable').off('click')
    ...
    this.promote = function (pieceSymbol) {
      ...
      if (pieceSymbol) {
        ...
        $.each(this.isWhite ? ['♘', '♗', '♖', '♕'] : ['♞', '♝', '♜', '♛'], function (index, pieceSymbol) {
          ...
          $('<li><div class="piece ' + pieceSymbols[pieceSymbol].color + ' ' + pieceSymbols[pieceSymbol].kind + '">' + pieceSymbol + '</div></li>').appendTo($('.promotion > ul')).on('click', function () {
  ...
  (function init () {
    ...
    $('td > .white.knight:first, td > .black.knight:first').css('transform', 'scaleX(-1)')
  ...
  return {
    ...
    deinit: function () {
      ...
      // Remove classes indicating field "ownership"
...
function Sync () {
  ...
  var gameName; var syncInterval; var windowHasFocus = 1
...
function Menu (name) {
  ...
  function init () {
    ...
    menuBox.on('mouseleave', function (handler) {
      highlightStripe(0)
    })
    stripes.each(function (index, element) {
      ...
      $(element).children().each(function (childIndex, childElement) {
        $(childElement).on('focus', function () {
          highlightStripe(index + 1)
          locked = 1
        })

        $(childElement).on('blur', function () {
          locked = 0
          highlightStripe(0)
        })
      })
    ...
    buttons.each(function (index, element) {
      $(element).on('click', function () { menu.menuOption(element.parentNode.id) })
    })
    ...
    menuOption: function (optionId) {
      ...
      switch (optionId) {
        ...
        case 'toggleAutoSave':
          ...
          // Set option
          controller.autoSave = !(controller.autoSave)

          // Save option
          if (controller.autoSave) {
            localStorage.noAutoSave = undefined
            game.updateLocalStorage()
          } else {
            localStorage.noAutoSave = true
          }
...
controller = (function () {
  ...
  (function init () {
    ...
    // Initialize "toggle awareness mode" button
    $('#toggleAwareness').on('click', function () {
      if ($('#chess').hasClass('awareness')) {
        $('#chess').removeClass('awareness')
      } else {
  ...
  return {
    autoSave: !((localStorage.noAutoSave && !sync)),
    ...
    spin: function (element, duration) {
      $(element).css('transition-property', 'transform').css('transition-duration', duration + 'ms').css('transition-timing-function', 'ease-out')
    ...
    flip: function (element, className, duration, callback) {
      // Flip and call the callback in the middle of the animation
      $(element).on('transitionend', function () {
        $(element).off('transitionend')

        $(element).css('transition-timing-function', 'ease-out').removeClass(className)
